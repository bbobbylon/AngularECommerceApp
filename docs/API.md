...
> 📖 **Live, interactive docs:** the backend serves an auto-generated OpenAPI 3.1 spec via
> [springdoc](https://springdoc.org). With the app running:
> - **Swagger UI** → <http://localhost:8585/swagger-ui.html> (try endpoints in-browser; click
>   **Authorize** to attach a Bearer token for the 🔒 order/admin endpoints)
> - **OpenAPI spec** → <http://localhost:8585/v3/api-docs> (JSON) · `/v3/api-docs.yaml` (YAML)
>
> This table is the curated overview; Swagger UI is the source-of-truth generated from the controllers.
> Disable both in production with `springdoc.api-docs.enabled=false` / `springdoc.swagger-ui.enabled=false`.
...
| [Headless API keys](#headless-api-keys) | GET/POST | `/admin/api-keys` | 🔒 JWT |
| | DELETE | `/admin/api-keys/{id}` | 🔒 JWT |
| [Webhooks](#webhooks) | GET/POST | `/admin/webhooks` | 🔒 JWT |
| | DELETE | `/admin/webhooks/{id}` | 🔒 JWT |
| | GET | `/admin/webhooks/{id}/deliveries` | 🔒 JWT |
...
```bash
curl "http://localhost:8586/api/countries"
# -> _embedded.countries: [{ id, code, name }]

curl "http://localhost:8586/api/states/search/findByCountryCode?code=US"
...
### Place an order
```bash
curl -X POST "http://localhost:8586/api/checkout/purchase" \
...
```bash
curl -X POST "http://localhost:8586/api/checkout/payment-intent" \
  -H "Content-Type: application/json" \
...
Full guide: **[WEBHOOKS.md](WEBHOOKS.md)**. A tenant issues itself a key and drives `/api/admin/**`
with no browser — the key resolves **both** tenant identity and Spring Security authorities.

```bash
# Issue (the raw key is returned exactly once — only its SHA-256 hash is stored)
curl -X POST "http://localhost:8585/api/admin/api-keys" -H 'Content-Type: application/json'   -d '{"name":"Warehouse sync","authorities":["Admin"],"expiresAt":null}'
# -> { id, name, keyPrefix, authorities, rawKey, expiresAt, dateCreated }

# Use it — no X-Tenant-Id needed; the key already identifies the tenant
curl "http://localhost:8585/api/admin/stats" -H 'X-Api-Key: lsk_...'

# Revoke (soft; evicts the lookup cache so it stops working immediately)
curl -X DELETE "http://localhost:8585/api/admin/api-keys/1"
```

An unknown/revoked/expired key returns the same generic **404** as an unknown tenant — no
enumeration signal. Header name is configurable via `app.api-key.header` (default `X-Api-Key`).

---

## Webhooks

Full guide: **[WEBHOOKS.md](WEBHOOKS.md)**. Register a URL for one or more event types and Luv2Shop
POSTs a **HMAC-SHA256-signed** JSON callback when they fire.

```bash
# Register (the signing secret is returned exactly once, masked on every later read)
curl -X POST "http://localhost:8585/api/admin/webhooks" -H 'Content-Type: application/json'   -d '{"url":"https://you.example.com/hooks","eventTypes":["order.created","shipment.shipped"],"active":true}'

# Delivery history for one subscription (paged envelope)
curl "http://localhost:8585/api/admin/webhooks/1/deliveries?page=0&size=10"
# -> { content:[{ id, webhookEventId, eventType, attemptNumber, outcome, httpStatusCode, errorMessage, attemptedAt }], totalElements, ... }
```

Event types: `order.created` · `shipment.shipped` · `shipment.delivered` · `return.approved` ·
`return.denied`.

Each callback carries `X-Webhook-Event` and `X-Webhook-Signature` (hex HMAC-SHA256 over the **raw
body**, keyed with the subscription secret — verify before parsing). Publishing is a transactional
outbox, so a rolled-back order never emits an event; a sweep (default every 30s) delivers with
backoff **1m → 5m → 15m → 60m → 360m**, then marks the event `ABANDONED`. Every attempt — success,
non-2xx, network error — is recorded in the delivery ledger.
...
- **CORS:** allowed origins `http://localhost:4200` and `http://localhost:4251`.

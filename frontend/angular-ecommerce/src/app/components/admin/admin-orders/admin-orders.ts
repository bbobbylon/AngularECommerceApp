...
export class AdminOrders implements OnInit {
  ...
  toggleFulfillment(order: AdminOrderView): void {
    ...
    this.admin.getShipments(order.id).subscribe({
      next: (shipments) => {
        this.shipmentsByOrder[order.id] = shipments;
        this.fulfillmentLoading.set(false);
      },
      error: () => {
        this.fulfillmentLoading.set(false);
        this.toast.error('Could not load shipments.');
      },
    });
    this.admin.getFulfillmentOptions(order.id).subscribe({
      ...
      error: () => {
        /* non-fatal — the warehouse dropdown just won't have a suggested default */
      },
    });
  }
  ...
  advanceShipment(
    ...
    this.admin
      .updateShipmentStatus(
        shipment.id,
        target,
        this.carrierFor[shipment.id],
        this.trackingFor[shipment.id],
      )
      .subscribe({
        next: (updated) => {
          this.shipmentsByOrder[order.id] = (this.shipmentsByOrder[order.id] ?? []).map((s) =>

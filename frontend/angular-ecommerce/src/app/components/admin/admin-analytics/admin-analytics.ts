...
export class AdminAnalytics implements OnInit {
  ...
  readonly maxTopProductRevenue = computed(() =>
    Math.max(1, ...this.topProducts().map((p) => p.revenue)),
  );
  readonly maxStatusCount = computed(() =>
    Math.max(1, ...this.statusBreakdown().map((s) => s.count)),
  );
  readonly totalOrders30d = computed(() =>
    this.revenue().reduce((sum, p) => sum + p.orderCount, 0),
  );
  ...
  ngOnInit(): void {
    ...
    this.admin
      ...
      .subscribe((statuses) => this.statusBreakdown.set(statuses));
  }
